...
class SharedBasis(nn.Module):
    """K rank-s basis factor pairs for one matrix type, shared across all layers."""
    def __init__(self, out_dim: int, in_dim: int, K: int, s: int):
        super().__init__()
        self.K = K
        # Store as (K*out_dim, s) and (K*in_dim, s) so Muon sees 2D matrices.
        self.P = nn.Parameter(torch.empty(K * out_dim, s))
        self.Q = nn.Parameter(torch.empty(K * in_dim, s))
        self.out_dim = out_dim
        self.in_dim = in_dim
        # Init: orthogonal per K-slice
        for k in range(K):
            nn.init.orthogonal_(self.P.data[k * out_dim : (k + 1) * out_dim])
            nn.init.orthogonal_(self.Q.data[k * in_dim : (k + 1) * in_dim])

    def get_factors(self) -> tuple[Tensor, Tensor]:
        """Return P as (K, out, s) and Q as (K, in, s)."""
        P = self.P.view(self.K, self.out_dim, -1)
        Q = self.Q.view(self.K, self.in_dim, -1)
        return P, Q
...
class HyperLoRALinear(nn.Module):
    ...
    def __init__(
        ...
        # LoRA factors
        if enable_lora:
            self.lora_A = nn.Parameter(torch.empty(out_dim, lora_rank))
    ...
    def materialize_weight(self, alpha: Tensor | None = None) -> Tensor:
        ...
        if self.enable_lora:
            scaling = self.lora_alpha_value / self.lora_rank
...
def _make_linear(out_dim: int, in_dim: int, hyper_cfg: dict | None, type_idx: int, layer_idx: int) -> nn.Module:
    """Create either a CastedLinear (dense mode) or HyperLoRALinear."""
    ...
    return HyperLoRALinear(
        ...
        enable_hyper=hyper_cfg["enable_hyper"],
    )
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.hyper = hyper_cfg is not None
        # Type indices: 0=c_q, 1=c_k, 2=c_v, 3=proj
        self.c_q = _make_linear(dim, dim, hyper_cfg, 0, layer_idx)
        self.c_k = _make_linear(kv_dim, dim, hyper_cfg, 1, layer_idx)
        self.c_v = _make_linear(kv_dim, dim, hyper_cfg, 2, layer_idx)
        self.proj = _make_linear(dim, dim, hyper_cfg, 3, layer_idx)
        if not self.hyper:
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int, layer_idx: int = 0, hyper_cfg: dict | None = None):
        super().__init__()
        hidden = mlp_mult * dim
        self.hyper = hyper_cfg is not None
        # Type indices: 4=fc, 5=proj
        self.fc = _make_linear(hidden, dim, hyper_cfg, 4, layer_idx)
        self.proj = _make_linear(dim, hidden, hyper_cfg, 5, layer_idx)
        if not self.hyper:
            self.proj._zero_init = True
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, layer_idx, hyper_cfg)
        self.mlp = MLP(dim, mlp_mult, layer_idx, hyper_cfg)
...
# 6 matrix types: c_q, c_k, c_v, attn.proj, mlp.fc, mlp.proj
NUM_MATRIX_TYPES = 6


def _build_hyper_cfg(args: Hyperparameters, model_dim: int, num_kv_heads: int, num_heads: int, mlp_mult: int) -> dict | None:
    ...
    matrix_shapes = [
        ...
        (kv_dim, model_dim),     # c_k
        (kv_dim, model_dim),     # c_v
        (model_dim, model_dim),  # attn.proj
...
class GPT(nn.Module):
    def __init__(
        ...
        self.hyper_mode = hyper_cfg is not None
        ...
        self.blocks = nn.ModuleList([
            Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base,
                  qk_gain_init, layer_idx=i, hyper_cfg=hyper_cfg)
...
def main() -> None:
    ...
    use_fullgraph = args.hyper_mode == "dense"
    compiled_model = torch.compile(base_model, dynamic=False, fullgraph=use_fullgraph)
    model: nn.Module = DDP(compiled_model, device_ids=[local_rank], broadcast_buffers=False) if distributed else compiled_model

    # Optimizer split — 5 groups for HyperLoRA, 3-4 for dense
    block_named_params = list(base_model.blocks.named_parameters())

    if args.hyper_mode == "dense":
        # Dense mode: same optimizer setup as baseline
        matrix_params = [
        ...
        # HyperLoRA mode: separate basis, lora, and block scalar params
        # Block params that are NOT part of HyperLoRALinear (attn_scale, mlp_scale, resid_mix, q_gain)
    ...
    if args.hyper_mode == "dense":
        ...
        # Basis params via Muon
        optimizer_basis = Muon(
            basis_params, lr=args.matrix_lr,
            momentum=args.muon_momentum, backend_steps=args.muon_backend_steps,
        )
        for group in optimizer_basis.param_groups:
            group["base_lr"] = args.matrix_lr
        # LoRA matrix params via Muon
        optimizer_lora = Muon(
            lora_matrix_params, lr=args.lora_lr,
            momentum=args.muon_momentum, backend_steps=args.muon_backend_steps,
        ) if lora_matrix_params else None
    ...
    while True:
        ...
        # Update Muon momentum for all Muon optimizers
        ...
        # Training strategy: selectively zero LR for frozen groups
        for opt in optimizers:
            for group in opt.param_groups:
                group["lr"] = group["base_lr"] * scale
        if args.hyper_mode != "dense" and args.train_strategy != "joint":
            train_frac = step / max(args.iterations, 1)
            p1_end = args.phase1_frac
    ...
    if args.hyper_mode != "dense" and args.export_mode == "factored":
        ...
        if master_process:
            ...
            log0(
                f"Serialized factored int8+zlib: {quant_file_bytes} bytes "
                f"(payload:{quant_stats['int8_payload_bytes']} raw_torch:{quant_raw_bytes} payload_ratio:{ratio:.2f}x)"
            )
            log0(f"Total submission size int8+zlib: {quant_file_bytes + code_bytes} bytes")

        # Roundtrip: reload, reconstruct weights, evaluate
        if distributed:
            dist.barrier()
