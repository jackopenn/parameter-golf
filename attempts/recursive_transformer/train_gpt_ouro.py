...
class Hyperparameters:
    ...
    # Ouro / LoopLM hyperparameters
    num_iterations = int(os.environ.get("NUM_ITERATIONS", 3))
    entropy_beta = float(os.environ.get("ENTROPY_BETA", 0.1))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,exit_gate,gate_bias",
    ).split(",")
    if pattern
)
...
class OuroGPT(nn.Module):
    """Ouro/LoopLM: weight-tied looped transformer with per-step exit gates."""
    ...
    def __init__(
        ...
        # Skip structure based on effective depth
        effective_layers = num_layers * num_iterations
    ...
    def _compute_logits_and_loss(self, x: Tensor, targets: Tensor) -> Tensor:
        """Compute LM head logits and CE loss from hidden states."""
        h_out = self.final_norm(x).reshape(-1, x.size(-1))
        if self.tie_embeddings:
            logits_proj = F.linear(h_out, self.tok_emb.weight)
        else:
            if self.lm_head is None:
                raise RuntimeError("lm_head is required when tie_embeddings=False")
            logits_proj = self.lm_head(h_out)
        logits = self.logit_softcap * torch.tanh(logits_proj / self.logit_softcap)
        return F.cross_entropy(logits.float(), targets, reduction="mean")

    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor | tuple[Tensor, list[Tensor]]:
...
def main() -> None:
    ...
    effective_layers = args.num_layers * args.num_iterations
    log0(f"ouro_looplm: {args.num_layers} blocks x {args.num_iterations} iterations = {effective_layers} effective layers")
    log0(f"entropy_beta:{args.entropy_beta}")
