"""Experimental branch-tail trainer forked from the baseline `train_gpt.py`."""
...
class Hyperparameters:
    ...
    val_loss_every = int(os.environ.get("VAL_LOSS_EVERY", "0" if single_h100_30m else "1000"))
    train_log_every = int(os.environ.get("TRAIN_LOG_EVERY", "50" if single_h100_30m else "200"))

    # Training length.
    iterations = int(os.environ.get("ITERATIONS", 20000))
    warmdown_iters = int(os.environ.get("WARMDOWN_ITERS", "3600" if single_h100_30m else "1200"))
    warmup_steps = int(os.environ.get("WARMUP_STEPS", 20))
    train_batch_tokens = int(os.environ.get("TRAIN_BATCH_TOKENS", 524_288))
    train_seq_len = int(os.environ.get("TRAIN_SEQ_LEN", 1024))
    max_wallclock_seconds = float(os.environ.get("MAX_WALLCLOCK_SECONDS", "1800.0" if single_h100_30m else "600.0"))
    ...
    branch_tail_layers = int(os.environ.get("BRANCH_TAIL_LAYERS", 2))
    teacher_mlp_mult = int(os.environ.get("TEACHER_MLP_MULT", 4))
    ...
    wandb_group = os.environ.get("WANDB_RUN_GROUP", "single-h100-30m" if single_h100_30m else "")
...
def ce_loss_from_logits(logits: Tensor, target_ids: Tensor) -> Tensor:
    return F.cross_entropy(logits.reshape(-1, logits.size(-1)).float(), target_ids.reshape(-1), reduction="mean")


def kd_kl_from_logits(student_logits: Tensor, teacher_logits: Tensor, temperature: float) -> Tensor:
    if temperature <= 0.0:
        raise ValueError(f"KD_TEMPERATURE must be positive, got {temperature}")
    student_flat = student_logits.reshape(-1, student_logits.size(-1))
    teacher_flat = teacher_logits.detach().reshape(-1, teacher_logits.size(-1))
    return F.kl_div(
...
def args_config(args: Hyperparameters) -> dict[str, object]:
    return {
        key: value
...
def maybe_init_wandb(args: Hyperparameters, *, master_process: bool):
    if not master_process or not args.wandb_enable:
        return None
    try:
        import wandb
    except ImportError as exc:
        raise RuntimeError("WANDB_ENABLE=1 requires `pip install wandb` in the active environment") from exc
...
def wandb_log(run, metrics: dict[str, object], *, step: int) -> None:
    if run is not None:
...
class BranchedTailGPT(nn.Module):
    def __init__(
        self,
        vocab_size: int,
        num_layers: int,
        ...
        num_kv_heads: int,
        mlp_mult: int,
        teacher_mlp_mult: int,
        branch_tail_layers: int,
        tie_embeddings: bool,
        tied_embed_init_std: float,
        logit_softcap: float,
        rope_base: float,
        ...
        self.tied_embed_init_std = tied_embed_init_std
        self.logit_softcap = logit_softcap
        self.branch_tail_layers = branch_tail_layers
        self.teacher_mlp_mult = teacher_mlp_mult
        self.tok_emb = nn.Embedding(vocab_size, model_dim)
        self.num_encoder_layers = num_layers // 2
        self.num_decoder_layers = num_layers - self.num_encoder_layers
        if not (0 <= branch_tail_layers <= self.num_decoder_layers):
            raise ValueError(
                f"BRANCH_TAIL_LAYERS={branch_tail_layers} must be in [0, {self.num_decoder_layers}] "
                f"for NUM_LAYERS={num_layers}"
            )
        self.num_shared_decoder_layers = self.num_decoder_layers - branch_tail_layers
        self.num_skip_weights = min(self.num_encoder_layers, self.num_decoder_layers)
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))
        self.blocks = nn.ModuleList(
            [
        ...
        self.teacher_tail_blocks = nn.ModuleList(
            [
                Block(
                    model_dim,
                    num_heads,
                ...
                for _ in range(branch_tail_layers)
            ]
        )
        self.teacher_skip_weights = nn.Parameter(torch.ones(branch_tail_layers, model_dim, dtype=torch.float32))
        self.final_norm = RMSNorm()
        self.lm_head = None if tie_embeddings else CastedLinear(model_dim, vocab_size, bias=False)
        if self.lm_head is not None:
            self.lm_head._zero_init = True
    ...
    def _forward_paths(self, input_ids: Tensor, *, teacher_enabled: bool) -> tuple[Tensor, Tensor | None]:
        x = self.tok_emb(input_ids)
        x = F.rms_norm(x, (x.size(-1),))
        x0 = x
        skips: list[Tensor] = []
        ...
        for i in range(self.num_encoder_layers):
            x = self.blocks[i](x, x0)
            skips.append(x)
        for i in range(self.num_shared_decoder_layers):
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            x = self.blocks[self.num_encoder_layers + i](x, x0)

        student_x = x
        if self.branch_tail_layers == 0 or not teacher_enabled:
            for tail_idx in range(self.num_shared_decoder_layers, self.num_decoder_layers):
                if skips:
                    student_x = student_x + self.skip_weights[tail_idx].to(dtype=student_x.dtype)[None, None, :] * skips.pop()
                student_x = self.blocks[self.num_encoder_layers + tail_idx](student_x, x0)
            return self._project_logits(student_x), None

        teacher_x = x
        student_skips = list(skips)
        teacher_skips = list(skips)
        for tail_idx in range(self.branch_tail_layers):
            decoder_idx = self.num_shared_decoder_layers + tail_idx
            if student_skips:
                student_x = (
                    student_x
                    + self.skip_weights[decoder_idx].to(dtype=student_x.dtype)[None, None, :] * student_skips.pop()
                )
            student_x = self.blocks[self.num_encoder_layers + decoder_idx](student_x, x0)
            if teacher_skips:
                teacher_x = (
                    teacher_x
                    + self.teacher_skip_weights[tail_idx].to(dtype=teacher_x.dtype)[None, None, :] * teacher_skips.pop()
                )
            teacher_x = self.teacher_tail_blocks[tail_idx](teacher_x, x0)
        return self._project_logits(student_x), self._project_logits(teacher_x)

    def student_export_state_dict(self) -> dict[str, Tensor]:
        return {
            name: tensor
            for name, tensor in self.state_dict().items()
            if not name.startswith("teacher_tail_blocks.") and name != "teacher_skip_weights"
        }
    ...
    def forward(
        ...
        *,
        ...
        student_logits, teacher_logits = self._forward_paths(input_ids, teacher_enabled=teacher_enabled)
        student_ce = ce_loss_from_logits(student_logits, target_ids)
        teacher_ce = torch.zeros((), device=student_logits.device, dtype=student_ce.dtype)
        kd_kl = torch.zeros((), device=student_logits.device, dtype=student_ce.dtype)
        if teacher_logits is not None:
            teacher_ce = ce_loss_from_logits(teacher_logits, target_ids)
            if kd_weight > 0.0:
                kd_kl = kd_kl_from_logits(student_logits, teacher_logits, kd_temperature)
        total = student_ce * student_ce_weight + teacher_ce * teacher_ce_weight + kd_kl * kd_weight
        if return_details:
            return total, student_ce.detach(), teacher_ce.detach(), kd_kl.detach()
        return total


def build_student_model(args: Hyperparameters, device: torch.device) -> GPT:
    model = GPT(
...
def main() -> None:
    ...
    if master_process:
        ...
        print(logfile)
    wandb_run = maybe_init_wandb(args, master_process=master_process)
    ...
    if args.branch_tail_layers > 0:
        base_model: nn.Module = BranchedTailGPT(
            vocab_size=args.vocab_size,
            num_layers=args.num_layers,
            model_dim=args.model_dim,
            num_heads=args.num_heads,
        ...
        base_model = build_student_model(args, device)
    ...
    if isinstance(base_model, BranchedTailGPT):
        block_named_params.extend((f"teacher_tail_blocks.{name}", param) for name, param in base_model.teacher_tail_blocks.named_parameters())
    matrix_params = [
        p
    ...
    if isinstance(base_model, BranchedTailGPT) and base_model.teacher_skip_weights.numel() > 0:
        scalar_params.append(base_model.teacher_skip_weights)
    ...
    export_param_count = (
        sum(t.numel() for t in base_model.student_export_state_dict().values())
        if isinstance(base_model, BranchedTailGPT)
        else sum(t.numel() for t in base_model.state_dict().values())
    )
    log0(f"model_params:{n_params}")
    log0(f"export_model_params:{export_param_count}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0("sdp_backends:cudnn=False flash=True mem_efficient=False math=False")
    log0(f"attention_mode:gqa num_heads:{args.num_heads} num_kv_heads:{args.num_kv_heads}")
    log0(
        f"use_compile:{args.use_compile} single_h100_30m:{args.single_h100_30m} "
    ...
    wandb_log(
        wandb_run,
        {
            ...
            "export_model_params": export_param_count,
    ...
    while True:
        ...
        teacher_ce_value = torch.zeros((), device=device)
        kd_kl_value = torch.zeros((), device=device)
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                if isinstance(base_model, BranchedTailGPT):
                    loss, student_ce, teacher_ce, kd_kl = model(
                        x,
                        y,
                    ...
                    student_ce_value += student_ce
                    teacher_ce_value += teacher_ce
                    kd_kl_value += kd_kl
                else:
                    loss = model(x, y)
                    student_ce_value += loss.detach()
            train_loss += loss.detach()
            (loss * grad_scale).backward()
        train_loss /= grad_accum_steps
        student_ce_value /= grad_accum_steps
        teacher_ce_value /= grad_accum_steps
        kd_kl_value /= grad_accum_steps
        ...
        tokens_per_sec = (step * args.train_batch_tokens) / max(approx_training_time_ms / 1000.0, 1e-9)
        if should_log_train:
            log0(
            ...
            wandb_log(
                wandb_run,
                {
                    ...
                    "student_ce": float(student_ce_value.item()),
                    ...
                    "peak_memory_allocated_mib": float(torch.cuda.max_memory_allocated() // 1024 // 1024),
                    "peak_memory_reserved_mib": float(torch.cuda.max_memory_reserved() // 1024 // 1024),
                },
                step=step,
            )
    ...
    export_state = (
        base_model.student_export_state_dict()
        if isinstance(base_model, BranchedTailGPT)
    ...
    if master_process:
        ...
        wandb_log(
            wandb_run,
            {
                "raw_model_bytes": float(model_bytes),
                "code_bytes": float(code_bytes),
                "total_submission_bytes": float(model_bytes + code_bytes),
            },
            step=step,
        )
    ...
    if master_process:
        ...
        wandb_log(
            wandb_run,
            {
                "int8_payload_bytes": float(quant_stats["int8_payload_bytes"]),
                "int8_zlib_model_bytes": float(quant_file_bytes),
                "int8_total_submission_bytes": float(quant_file_bytes + code_bytes),
            },
            step=step,
        )
    ...
    eval_model = build_student_model(args, device)
    ...
    wandb_log(
        wandb_run,
        {
            "final_val_loss": float(q_val_loss),
            "final_val_bpb": float(q_val_bpb),
            "final_eval_time_ms": float(1000.0 * (time.perf_counter() - t_qeval)),
        },
        step=step,
    )
